    ...
    if (pv := event.is_private) or event.is_group :
        if pv:
            try:
                ...
                await event.reply(f"برای استفاده از ربات ابتدا در کانال زیر عضو شوید\n\n@{Config.CHANNEL_USERNAME}\n\nبعد از عضویت مجدداً ربات را /start نمایید.")
                return
        ...
        if event.file :
            ...
            await event.reply(f"📥 لینک دانلود فایل : \n\n{Config.DOMAIN}/{id}\n\n🤖 : {bot_url}")
            return
            ...
            if id_hex := id_msg.group(1) :
                ...
                if regex := re.search(r"(\d*)/(\d*)",msg.message):
                    if user_id := int(regex.group(1)) :
                        ...
                        bot_url = f"t.me/{username_bot}?start={id_hex}"
                        forward_reply = await forward.reply(f"21 ثانیه دیگر متن پاک خواهد شد\n\n📥 لینک دانلود فایل : \n\n {Config.DOMAIN}/{id_name}\n\n🤖 : {bot_url}",link_preview=False)
                        await asyncio.sleep(12)
                        await forward_reply.edit(f"10 ثانیه دیگر متن پاک خواهد شد\n\n📥 لینک دانلود فایل : \n\n {Config.DOMAIN}/{id_name}\n\n🤖 : {bot_url}")
        ...
        if pv:
            await event.reply("📤 برای دریافت لینک دانلود مستقیم فایل مورد نظر خودتون رو ارسال یا فوروارد کنید.")
